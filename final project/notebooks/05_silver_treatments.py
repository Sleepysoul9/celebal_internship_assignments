...
# Notebook: 05_silver_treatments
# Author  : Kushagra Sanghi
#
# Purpose :
#     Transform Bronze Treatment data into the Silver Layer
#     by applying data quality rules, business validation,
#     and SCD Type 2.
# ==========================================================
...
SOURCE_TABLE = "bronze_treatments"

TARGET_TABLE = "silver_treatments"

# COMMAND ----------

# Read Bronze Treatment Data

print("=" * 60)
print("Loading Bronze Treatment Data")
print("=" * 60)
...
print("=" * 60)
print("Validating Bronze Treatment Data")
print("=" * 60)
...
# Cleaning and Standardizing Treatment Data

print("=" * 60)
print("Cleaning and Standardizing Treatment Data")
print("=" * 60)
...
silver_df = (
    ...
    .withColumn(
        "appointment_id",
        trim(col("appointment_id"))
    )

    .withColumn(
        "treatment_type",
        upper(trim(col("treatment_type")))
    )
...
# Remove Records with NULL Treatment ID

silver_df = (

    silver_df

    .filter(col("treatment_id").isNotNull())

)

# Remove Duplicate Treatment Records
...
print("✓ Treatment Type Standardized.")

print("✓ Duplicate Treatment Records Removed.")
...
valid_records = (
    ...
    .filter(col("treatment_valid") == True)
...
invalid_records = (
    ...
    .filter(col("treatment_valid") == False)

    .count()

)

print(f"Valid Treatment Records   : {valid_records}")

print(f"Invalid Treatment Records : {invalid_records}")
    ...
    .withColumn(
        ...
        sha2(
            ...
            concat_ws(
                ...
                coalesce(col("appointment_id"), lit("")),

                coalesce(col("treatment_type"), lit("")),

                coalesce(col("description"), lit("")),

                coalesce(col("cost").cast("string"), lit("")),

                coalesce(col("treatment_date").cast("string"), lit(""))
...
if not table_exists:
    ...
    new_df = (
        ...
        .filter(col("old.treatment_id").isNull())
    ...
    changed_df = (
        ...
        .filter(
            ...
            (col("old.treatment_id").isNotNull())
...
print("=" * 60)
print("Persisting Silver Treatment Table")
print("=" * 60)
...
if not table_exists:
    ...
    print("Creating Silver Treatment Table...")
...
print("✓ Silver Treatment Table Persisted Successfully.")
...
audit_entry = Row(
    ...
    remarks = "Silver Treatment Pipeline Completed Successfully.",
...
null_key_count = (
    ...
    .filter(col("treatment_id").isNull())
...
print(f"Duplicate Treatment IDs                : {duplicate_count}")
...
print(f"NULL Treatment IDs                     : {null_key_count}")
...
print("=" * 60)
print("Silver Treatment Pipeline Completed Successfully.")
print("=" * 60)
